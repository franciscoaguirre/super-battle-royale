...
- **Replicated protocol** is registered once in `net::register_protocol`: components `NetPos`, `Player`, `PlayerColor`, `Health`, `Bot` (all derive `Serialize`/`Deserialize`); client-event `PlayerInput`. Replicon hashes this and rejects mismatched clients. **Anything you want replicated must be added here and derive serde.**
...
The integration test `tests/replication.rs` (run with `cargo test --features server`) connects a headless client+server over loopback and asserts replication, input, and the shoot→projectile flow end-to-end, plus direct tests of the damage→death combat loop for both players and bots.
