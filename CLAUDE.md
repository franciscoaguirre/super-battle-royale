...
- **Movement is split into intent + apply.** `PlayerIntent` (server-only) holds the desired direction; `apply_player_intent` (authoritative, `FixedUpdate`) moves `NetPos` via `step_player`. Bots use `BotIntent`/`apply_bot_intent` (still `Update`; bots aren't predicted), driven by a simple AI that hunts the nearest player. Offline writes intent from local keys (`read_local_input`). Online, the server buffers each client's inputs in a per-player `InputQueue` (server-only) and `dequeue_inputs` (`FixedUpdate`, `.before(apply_player_intent)`) applies **exactly one input per tick**, recording the applied seq in the replicated `LastProcessedInput`. The dir-clamp in `step_player` is the speed-hack guard.
